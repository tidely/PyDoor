...
def stoptask(client: Client, task: Task):
    ...
    logging.debug(
        "Attempting to stop task (%s) on client (%s)", task.identifier, client.port
    )
...
def output(client: Client, task: Task) -> str:
    ...
    logging.debug(
        "Task (%s) returned output from client (%s)", task.identifier, client.port
    )
...
def find(task_list: list[Task], task_id: str) -> Union[Task, None]:
    ...
    return next(
        (task for task in task_list if task.identifier.startswith(task_id)), None
    )
