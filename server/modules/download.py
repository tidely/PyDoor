...
def download(
    client: Client, url: str, filename: str, timeout: Union[float, None] = 30.0
):
    """Make the client download a file"""
    logging.debug(
        "Downloading file '%s' from '%s' to client (%s)", filename, url, client.port
    )
    ...
    logging.info(
        "Downloaded file '%s' from '%s' to client (%s)", filename, url, client.port
    )
