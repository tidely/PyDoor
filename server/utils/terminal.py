...
def task_print(tasks: list[Task]):
    ...
    print("-Tasks-")
    print(
        f"State: {Fore.GREEN}Running{Fore.WHITE} - {Fore.RED}Stopped with output{Fore.WHITE}\n"
    )
    for task in tasks:
        ...
        status = (
            f"{Fore.GREEN if task.native_id else Fore.RED}{task.command}{Fore.WHITE}"
        )
...
def make_prompt(client: Client, cwd: str) -> str:
    ...
    match client.system:
        ...
        case "Linux":
            prompt = (
                f"{client.user}@{client.hostname}:{cwd.replace(client.home, '~')}$ "
            )
        case "Darwin":
            if cwd == client.home:
                folder = "~"
